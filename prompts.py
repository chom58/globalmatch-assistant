...
【Markdown書式ルール - 厳守】
以下のルールに違反すると画面上で `*` が literal な記号として表示されてしまい、可読性を著しく損ないます。必ず守ってください。

1. 箇条書きの行頭記号は **必ず `-`** を使う（`*` および `•` は使用禁止）
2. **複数の項目を同じ行に並べない**。以下のような出力は**絶対禁止**：
   - NG例: `* 課題: ... * 解決策: ... * 技術スタック: ... * 成果: ...`
   - NG例: `- 日本語：N1 - 英語：ビジネスレベル - その他：中国語`
3. 各 `-` 項目は **必ず行頭（改行後）** に置き、項目ごとに改行する
4. 「代表プロジェクト」「職務経歴の担当業務」「語学」「プロジェクト概要」など、複数要素を持つブロックは以下の形で**各行を独立**させる：
   ```
   - **課題**: ...
   - **解決策**: ...
   - **技術スタック**: ...
   ...
   - **チーム体制**: ...
   ```
5. 文の途中に `*` を入れない（強調は `**太字**` の形でのみ使用）
6. インラインで箇条書きを表現したい場合は `-` や `*` ではなく読点「、」または全角スラッシュ「／」を使う
